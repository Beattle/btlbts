...
$arrFg=array("IBLOCK_ID"=>12,"ACTIVE"=>"Y","PROPERTY_GOOD"=>$arItem["ID"]);
$eres = CIBlockElement::GetList(Array(), $arrFg);
$arResult['ITEMS'][$key]["COMMENT_NUM"]=intval($eres->SelectedRowsCount());

$arResult['ITEMS'][$key]["SECT"]=array();
	if(intval($arItem["IBLOCK_SECTION_ID"])>0)
	{
		
		$sres = CIBlockSection::GetList(Array($by=>$order), array("IBLOCK_ID"=>$arParams["IBLOCK_ID"],"ID"=>$arItem["IBLOCK_SECTION_ID"]), true);
		if($sar_res = $sres->GetNext())
		{
			$arResult['ITEMS'][$key]["SECT"][]=$sar_res;
			if(intval($sar_res["SECTION_ID"])>0)
			{
				$sres = CIBlockSection::GetList(Array($by=>$order), array("IBLOCK_ID"=>$arParams["IBLOCK_ID"],"ID"=>$sar_res["SECTION_ID"]), true);
				if($sar_res = $sres->GetNext())
				{
					$arResult['ITEMS'][$key]["SECT"][]=$sar_res;
				}
...
foreach ($arResult['ITEMS'] as $key => $arItem)
{
	...
	foreach ($arItem["OFFERS"] as $off=>$offers)
	{
		...
		if($offers["PRICES"][$arParams["PRICE_CODE"][0]]["VALUE"]>$max) {
			...
			$curr=$offers["PRICES"][$arParams["PRICE_CODE"][0]]["CURRENCY"];
		}

	}
	$arResult['ITEMS'][$key]["MAX_PRICE"]["VALUE"]=$max;
	$arResult['ITEMS'][$key]["MAX_PRICE"]["CURRENCY"]=$curr;
	$arResult['ITEMS'][$key]["MAX_PRICE"]["PRINT_VALUE"]=$maxd;
	$arResult['ITEMS'][$key]["MAX_PRICE"]["DISCOUNT_VALUE"]=$dics;
	$arResult['ITEMS'][$key]["MAX_PRICE"]["PRINT_DISCOUNT_VALUE"]=$dicsd;
	$arResult['ITEMS'][$key]["MAX_PRICE"]["ID"]=$offid;
	$arResult['ITEMS'][$key]["MAX_PRICE"]["ADD_URL"]=$offurl;
