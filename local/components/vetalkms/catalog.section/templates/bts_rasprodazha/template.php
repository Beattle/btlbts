	...
	<div class="bl_product3">
					...
					<?
						if(!empty($arItem['PREVIEW_PICTURE']['ID']))
						{
							...
							$img["src"] = "/upload/no-photo_140x100.jpg";
						}
					?>
					...
					<div class="bl_product3_ttle">
						...
						<div class="middle_side">
							<div class="chars">
								...
								if(count($arItem['DISPLAY_PROPERTIES']["characteristics"]["VALUE"]) > 0) {
									$tab1 .= '<table>';
									foreach($arItem['DISPLAY_PROPERTIES']["characteristics"]["VALUE"] as $key => $val) {
										$tab1 .= '<tr><td>' . $val .'</td><td>' . $arItem['DISPLAY_PROPERTIES']["characteristics"]["DESCRIPTION"][$key] . '</td></tr>';
									}
									$tab1 .= '</table>';
								}
								echo $tab1;
