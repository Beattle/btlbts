...
$arResult["SKIP_FIRST_STEP"] = (($arResult["POST"]["SKIP_FIRST_STEP"] == "Y") ? "Y" : "Y");
$arResult["SKIP_SECOND_STEP"] = (($arResult["POST"]["SKIP_SECOND_STEP"] == "Y") ? "Y" : "N");
$arResult["SKIP_THIRD_STEP"] = (($arResult["POST"]["SKIP_THIRD_STEP"] == "Y") ? "Y" : "Y");
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && strlen($_REQUEST["backButton"]) > 0 && ($arParams["DELIVERY_NO_SESSION"] == "N" || check_bitrix_sessid()))
{
	...
	if($arResult["CurrentStep"]==6) $arResult["CurrentStep"]=4;
	if($arResult["CurrentStep"]==5) $arResult["CurrentStep"]=1;
...
//if(!empty($_REQUEST["contButton"]))
{
	...
	if ($arResult["CurrentStep"] > 0 && $arResult["CurrentStep"] <= 6)
	{
		...
		{
			LocalRedirect("/personal/order/?CurrentStep=7&ORDER_ID=" . $_GET['ORDER_ID']);
		...
		while ($arProperties = $dbProperties->GetNext())
		{
			...
			elseif ($arProperties["TYPE"] == "LOCATION")
			{
				...
				$dbVariants = CSaleLocation::GetList(
						...
						false,
						false,
						array("ID", "COUNTRY_NAME", "REGION_NAME", "CITY_NAME", "SORT", "COUNTRY_NAME_LANG", "CITY_NAME_LANG")
					);
				while ($arVariants = $dbVariants->GetNext())
				{
