...
if($_SERVER["REQUEST_METHOD"] == "POST" && strlen($_POST["submit"]) > 0 && $_POST["remont"] == "y")
{
	...
	{	
		...
		if(strlen($_POST["neispravnost"]) <= 1)
		{
			$arResult["ERROR_MESSAGE"][] = "Вы не ввели Описание неисправности и причины ее возникновения";
		}
		...
		if(!isset($arResult["ERROR_MESSAGE"]))
		{
			...
			//$message .= "ИНН - ".$_POST["inn"]."<br />";
			$message .= "Адрес  - ".$_POST["adres"]."<br />";
			//$message .= "Дата и номер договора - ".$_POST["date"]."<br />";
			...
			$message .= "Заводской номер - ".$_POST["nomer"]."<br />";
			$message .= "Неисправность - ".$_POST["neispravnost"]."<br />";			
			$message .= "<br /><br /><br />";
			...
			//$mail->AddAddress($arParams["EMAIL_TO"]);
			...
			$uploaddir = '/var/www/webuser/data/www/btlbts/upload/';
			$i = 0;
...
*/				
			LocalRedirect($APPLICATION->GetCurPageParam("success=Y&act=remont", Array("success","act")));
		}
...
elseif($_REQUEST["success"] == "Y" && $_REQUEST["act"] == "remont")
{
