	...
	foreach($arCategory["ITEMS"] as $i => $arItem)
	{
		if(isset($arItem["ITEM_ID"]))
		{
            ...
            $propObj = CIBlockElement::GetProperty(
                ...
                $arItem['ITEM_ID'],
                array("sort" => "asc"),
                Array("CODE"=>"second_hand")
            );
            while ($ob = $propObj->GetNext()) {
                if($ob['VALUE_ENUM'] === 'Y'){
                    $arResult['CATEGORIES'][$category_id]['ITEMS'][$i]['URL'] = '/catalogue_bu'.$arCategory["ITEMS"][$i]['URL'];
                }else{
                    $arResult['CATEGORIES'][$category_id]['ITEMS'][$i]['URL'] = '/catalogue'.$arCategory["ITEMS"][$i]['URL'];
                }
...
if(CModule::IncludeModule("catalog") && CModule::IncludeModule("iblock")) {
    $db_old_groups = CIBlockElement::GetElementGroups($arItemIDs, true);
    $sectionsIDs = array();
    while ($ar_group = $db_old_groups->Fetch()) {
        if (!in_array($ar_group['ID'], $sectionsIDs)) {
            $sectionsIDs[] = $ar_group['ID'];
            $group = CIBlockSection::GetByID($ar_group['ID']);
            if ($arGroup = $group->GetNext()) {
                $arResult['CATS'][] = $arGroup;
            }
        }
    }
}
