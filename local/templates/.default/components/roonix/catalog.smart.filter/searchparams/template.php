	...
	<?endforeach;?>
		...
		foreach($arResult["ITEMS"] as $k=>$arItem):?>
			...
			<div class="sliderui_item">
							...
							<select name="SECTION_ID" id="fsect" onchange="$('.smartfilter').submit();">
							<option value=""></option>
							<?
							$arrF=array("IBLOCK_ID"=>1,"DEPTH_LEVEL"=>1,"ACTIVE"=>"Y");
							$db_list = CIBlockSection::GetList(Array("SORT"=>"ASC"), $arrF);
							while($ar_result = $db_list->GetNext())
							{


							?>
								<option value="<?=$ar_result["ID"]?>" <?=($_REQUEST["SECTION_ID"]==$ar_result["ID"]?"selected":"")?>><?=$ar_result["NAME"]?></option>
								<?}?>
						...
						<div class="sliderui_item">
							...
							<select name="PSECTION_ID" onchange="$('.smartfilter').submit();">
								<option value=""></option>
								<?
								if(intval($_REQUEST["SECTION_ID"])>0)
								{
								$arrF=array("IBLOCK_ID"=>1,"DEPTH_LEVEL"=>2,"ACTIVE"=>"Y","SECTION_ID"=>$_REQUEST["SECTION_ID"]);
							$db_list = CIBlockSection::GetList(Array("SORT"=>"ASC"), $arrF);
							while($ar_result = $db_list->GetNext())
							{
							...
							?>
								<option value="<?=$ar_result["ID"]?>" <?=($_REQUEST["PSECTION_ID"]==$ar_result["ID"]?"selected":"")?>><?=$ar_result["NAME"]?></option>
								<?}
								}
								?>
			...
			<div class="sliderui_item">
					...
					$(function() {
						...
						if($arItem["PROPERTY_TYPE"] != "N" && !isset($arItem["PRICE"]) && intval($_REQUEST["PSECTION_ID"])>0 && !empty($arItem["VALUES"])):
		...
		?>
		<div id="valdf" style="display:none;">
			<?
			if(empty($_REQUEST["del_filter"])) echo $str;?>
