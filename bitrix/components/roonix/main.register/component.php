...
if($_REQUEST["group"]==6)
{
$arResult["REQUIRED_FIELDS"]=array_merge($arResult["REQUIRED_FIELDS"],array("NAME"));

if(empty($_REQUEST["UF_BANK"])) $arResult["ERRORS"][]='Поле "Наименование банка" обязательно для заполнения';
if(empty($_REQUEST["UF_UADDRESS"])) $arResult["ERRORS"][]='Поле "Юридический адрес" обязательно для заполнения';

}
else
{
	$arResult["REQUIRED_FIELDS"]=array_merge($arResult["REQUIRED_FIELDS"],array("PERSONAL_PHONE","NAME","LAST_NAME"));
}
	...
	foreach ($arResult["SHOW_FIELDS"] as $key)
	{
	 if($key=="LOGIN")
	 {
		 ...
		 $_REQUEST["REGISTER"][$key]=$_REQUEST["REGISTER"]["EMAIL"];
		 
		  $arResult["VALUES"][$key]=$_REQUEST["REGISTER"]["EMAIL"];
	...
	else // if there;s no any errors - create user
	{
		...
		if($_REQUEST["group"]==6) $arResult['VALUES']["GROUP_ID"]=6;
