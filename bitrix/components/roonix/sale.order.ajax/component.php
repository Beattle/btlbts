...
if ($arParams["ALLOW_AUTO_REGISTER"] == "Y" )
{
	if(strlen($_REQUEST["ORDER_ID"]) <= 0)
	{
		...
		if($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["confirmorder"]) && ($arParams["DELIVERY_NO_SESSION"] == "N" || check_bitrix_sessid()))
		{
			...
			if(IntVal($_POST["PERSON_TYPE_OLD"]) == $arUserResult["PERSON_TYPE_ID"])
			{
				...
				if($_POST["next"]=="Y") $arUserResult["FINAL_STEP"] = "Y";
		...
		//if (IntVal($arUserResult["DELIVERY_LOCATION"]) > 0)
		{
			...
			if(intval($arUserResult["DELIVERY_LOCATION"])>0)  $arFl["LOCATION"]=$arUserResult["DELIVERY_LOCATION"];
		...
		if($arUserResult["FINAL_STEP"]=="Y" && $_POST["next"]!="Y")
		{
			...
			if($arUserResult["CONFIRM_ORDER"] == "Y" && empty($arResult["ERROR"]))
			{
				...
				if(empty($arResult["ERROR"]))
				{
					...
					if(intval($_REQUEST["receive_address"])>0 && intval($arResult["ORDER_ID"])>0)
					{
					...
					if (empty($arResult["ERROR"]))
					{
						CSaleOrderUserProps::DoSaveUserProfile(5, $arUserResult["PROFILE_ID"], $arUserResult["PROFILE_NAME"], $arUserResult["PERSON_TYPE_ID"], $arUserResult["ORDER_PROP"], $arResult["ERROR"]);
					}
					...
					if (empty($arResult["ERROR"]))
					{
						...
						"EMAIL" => (strlen($arUserResult["USER_EMAIL"])>0 ? $arUserResult["USER_EMAIL"] : ""),
		...
		else $arUserResult["CONFIRM_ORDER"] = "N";
	...
	else
	{
		...
		if ($arOrder && $arOrder["USER_ID"] == IntVal(5))
		{
